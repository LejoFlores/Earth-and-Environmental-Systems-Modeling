...
#### Jupyter Notebook Programming Assignments (60%)
...
Peer review in this course is designed to function as an ongoing peer-mentoring relationship rather than a one-off grading exercise. Over the semester you will be paired with classmates to read, run, and constructively critique each other's notebooks—checking not just whether the code works, but whether the modeling choices are sound, the documentation is clear, and the results are interpreted carefully—so that everyone develops the habit of giving and receiving the kind of feedback that strengthens scientific software.  
